...
In contrast, in future lecture, we will also discuss 
...
exprs <- read_csv("../../2018-working/data/GSE5859_exprs.csv")
...
dendrogram above. From the dendrogram we can decipher 
...
are spread across all three clusters. This is a 
surprising result and we will continue to explore 
this to see what's going on. 
...
new.dat %>% 
  ggplot(aes(x=X1, y=X2, color = ethnicity )) + geom_point()
```
...
(bote that the columns are are centered by default)
...
```{r}
new.dat %>% 
  ggplot(aes(x=exprs_date, y=PC1)) + geom_point() + 
  ggtitle("Relationship between the PC1 and the date the samples were processed") + 
  xlab("Date sample was processed") + 
  ylab("Principal Component 1") 
```

Around what day do you notice a difference in
